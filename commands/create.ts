...
export async function CreateProject(
  ...
  try {
    ...
    const indexHtmlFile = await Deno.create(
      join(`${path}/${name}/public`, "index.html"),
    );
    const settings = await Deno.create(
      join(`${path}/${name}/.vscode`, "settings.json"),
    );
    const defaultConfig = await Deno.create(
    ...
    console.log(
      `Project created successfully in ${(endTime - startTime) / 1000}s.! ✨`,
    );
